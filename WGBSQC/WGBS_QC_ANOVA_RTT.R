...
##############################################################################
#RTT samples
path <- "/Users/annieciernia/Box\ Sync/LaSalle\ Lab/Experiments/humanASD_WGBSDMRs/WGBS_QC/Rett"
setwd(path)
...
PerMe$Sample <- gsub("_1_val_1","",PerMe$Sample)
...
DF$Sample <- gsub("_1_val_1","",DF$Sample)
...
write.xlsx(ANOVAout, file = "RTT_RM-ANOVA_PercentMethylation.xlsx")
...
PerMemerge2$Group <- gsub("Control_RTT","Control",PerMemerge2$Group)
PerMemerge2$Group <- as.factor(PerMemerge2$Group)
PerMemerge2$Group <- relevel(PerMemerge2$Group,ref="Control")

pdf("RTT_GLobaMethylation.pdf", height = 3, width =6)    # create PNG for the heat map       
...
TTESTout$cohort <- c("RTT")

#setwd("/Users/annieciernia/Box\ Sync/LaSalle\ Lab/Experiments/humanASD_WGBSDMRs/WGBS_QC/RTT")
write.xlsx(TTESTout,file="RTT_WGBSQC_T-test.xlsx",row.names = F)

mergeDF <- merge(sampleinfo,DFtest,by.y="Sample",by.x="filename",all.y=T)
mergeDF2 <- merge(mergeDF,DFout1,by.x="filename",by.y="Sample")

write.xlsx(mergeDF2,file="RTT_WGBS_sampleinfo.xlsx")
