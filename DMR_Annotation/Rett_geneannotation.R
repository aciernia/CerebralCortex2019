...
#Rett DMRs
DMRgenes <- read.delim("RTT_DMRs_closestgenes.txt",header=F, sep="\t", stringsAsFactors=FALSE)
...
ggsave(plot = plot, filename="Rett_Log10CountHistogram_Genes10kb.pdf",width = 8, height = 5, dpi = 300, useDingbats = FALSE)
...
write.csv(DMRgenes_filter, file="RettDMRgenes_within10kb.csv")

BGgenes_filter <- DMRinfomerge %>% filter(label == "BG Region Genes")#48852
write.csv(BGgenes_filter, file="BGregiongenes_within10kb.csv")
...
write.table(DMRgenes_filter,file='Rett_DMR_EnsemblHG38.txt',sep="\t", quote=FALSE,col.names = F,row.names = F)

write.table(BGgenes_filter,file='ConsensusBG_DMR_EnsemblHG38.txt',sep="\t", quote=FALSE,col.names = F,row.names = F)
...
#save to file
write.csv(DMRcounts, "Rett_Counts_DMR-gene_perlocation.csv",quote=F, row.names = F)
...
ggsave(filename="Rett_DMR-genes_GeneomicLocation.pdf",width = 8, height = 5, dpi = 300,  useDingbats = FALSE)
...
#write to file
write.csv(out,"Rett_TwoTailFishersExact_DMRgenes_GenomicContext.csv",quote=F, row.names = F)
write.csv(counts2,"Rett_DMRgenecounts_GenomicContex.csv",quote=F, row.names = F)
